...
しかし、ブラウザごとに解釈方法や表現方法が異なる場合があるため、「Chrome ではきれいに表示されているが、Edge ではレイアウトが崩れている」「Safari と Firefox で挙動が微妙に異なる」といったことがよく発生する
...
属性に設定する内容のことを **値** と呼び、`属性="値"` という形で記述する（値はダブルクオーテーション `"` だけでなくシングルクオーテーション `'` で囲んでも良いし、そもそも囲まなくても良い場合もあるが、基本的にはダブルクオーテーションで囲むように統一しておくのが望ましい）
...
見出しタグは大見出しとなる `<h1>` から順に使っていき、次の中見出しには `<h2>`、その次の小見出しには `<h3>` ... のように、数字の順番に沿って使っていくのが望ましい
...
- HSL(HSLA)値で指定:
    - hue（色相）, saturation（彩度）, lightbess（輝度）の数値を組み合わせた表現方法で `hsl(色相, 彩度, 輝度)` で指定する
        - 色相: 色相環における色の角度を 0～360 の数値で指定する
            - https://ja.wikipedia.org/wiki/%E8%89%B2%E7%9B%B8
        - 彩度: 色の鮮やかさを `0%`～`100%` の割合で指定し、「0%」が灰色、「100%」が純色となる
        - 輝度: 光源の明るさを `0%`～`100%` の割合で指定し、「0%」が黒、「50%」が純色、「100%」が白となる
    - `rgba` と同様にAlpha値の指定ができる `hsla(色相, 彩度, 輝度, 不透明度)` という書き方も可能
    - 記述例: `div { background-color: hsla(270,100%,80%,0.5); }`

##### カラーコードでの不透明度指定について
カラーコード4桁もしくは8桁で、不透明度指定も可能

8桁指定の場合、7～8桁目が不透明度となり、「00」で完全透明、「ff」で完全不透明となる

例えば `#ff00007f` のように記述すると、半透明な赤色を指定できる

赤・緑・青・不透明度それぞれ、同じ数値が連続する場合は4桁に省略可能（`#ff0000ff` => `#f00f`）

なお、[2019年の情報](https://qiita.com/mash8708/items/df25076d231b3f49bbf6)として Internet Explorer および Edge で8桁カラーコードが使えないという情報があるが、2021年8月現在、Microsoft Edge では問題なく使えそうだったため、おそらく殆どのモダンブラウザはすでに対応していると思われる

とは言え、すべてのブラウザを調べたわけではないため、使用する場合は事前に取り決めた標準ブラウザで使用可能かどうか確認することを推奨する
...
/* ヘッダー領域 */
header {
    background-color: #c3b79d; /* 背景色を薄茶色に */
